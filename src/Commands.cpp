...
void CommandUtils::Record(int c, const VulkanBackend::BackendData& backendData, HawkEye::Pipeline::Private* pipelineData)
{
	...
	if (pipelineData->passData[0].type == PipelinePass::Type::Computed)
	{
		...
		descriptorSets.push_back(pipelineData->passData[0].frameDescriptors[c].descriptorSet);
		if (pipelineData->passData[0].descriptorData.descriptorSet != VK_NULL_HANDLE)
		{
			descriptorSets.push_back(pipelineData->passData[0].descriptorData.descriptorSet);
		}
		...
		{
			vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, pipelineData->passData[0].pipelineLayout, 0,
