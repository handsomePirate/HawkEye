	...
	void* windowHandle, void* windowConnection)
{
	...
	if (layers[0].type == PipelineLayer::Type::Rasterized)
	{
		...
		{
			if (layers[0].shaders[s].first == PipelineLayer::Shader::Vertex)
			{
				shaderStages[0].module = VulkanShaderCompiler::Compile(device, layers[0].shaders[s].second.c_str());
				p_->shaderModules.push_back(shaderStages[0].module);
			}
			else if (layers[0].shaders[s].first == PipelineLayer::Shader::Fragment)
			{
				shaderStages[1].module = VulkanShaderCompiler::Compile(device, layers[0].shaders[s].second.c_str());
				p_->shaderModules.push_back(shaderStages[1].module);
			}
			else
			{
				CoreLogError(VulkanLogger, "Pipeline: A rasterization pipeline can only use vertex and fragment shaders. Provided \'%s\'.",
